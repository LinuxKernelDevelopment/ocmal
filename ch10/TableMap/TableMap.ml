module type TableMap = sig
  ...
  (** [insert k v m] mutates map [m] to bind [k] to [v]. If [k] was
      already bound in [m], that binding is replaced by binding to
      [v]. *)
  ...
  (** [of_list hash lst] creates a map with the same bindings as [lst],
      using [hash] as hash function. Requires: [lst] does not
...
module HashMap : TableMap = struct
  ...
  (** [rehash tab new_capacity] replaces the buckets arrayof [tab] with a new
  ...
  let remove k tab = assert false

  let bindings tab = assert false

  let of_list hash lst = assert false
end